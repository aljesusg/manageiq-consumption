...
RSpec.describe ManageIQ::Consumption::DataView, :type => :model do
  ...
  context 'basic life cycle' do
    let(:charge) { FactoryGirl.build(:data_view) }
  ...
  context '#stored data' do
    let(:charge_data) { FactoryGirl.build(:data_view, :with_data_snapshot) }
    ...
    it "stored event" do
      ...
      charge1 = FactoryGirl.create(:data_view,
